...
function renderResults(sectionResults, topicResults = []) {
  ...
  if (topicResults.length > 0) {
    ...
    topicResults.forEach(topic => {
      ...
      card.innerHTML = `
        ...
        <div>${(topic.commonTerms || []).slice(0, 5).map(term => `<span class="badge">${escapeHtml(term)}</span>`).join("")}</div>
      `;
  ...
  sectionResults.forEach(section => {
    ...
    card.innerHTML = `
      ...
      <div>${(section.keywords || []).slice(0, 4).map(keyword => `<span class="badge">${escapeHtml(keyword)}</span>`).join("")}</div>
    `;
...
function renderTopic(topic) {
  ...
  detailView.innerHTML = `
    ...
    <div>
      ${(topic.commonTerms || []).map(term => `<span class="badge">${escapeHtml(term)}</span>`).join("")}
    </div>
